...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    obs_minmax = max(preds_nn[[i]][[j]]$Y) - min(preds_nn[[i]][[j]]$Y)
    NRMSE = sqrt(mean((preds_nn[[i]][[j]]$yh - preds_nn[[i]][[j]]$Y)^2))/obs_minmax
    NRMSEs_nn[i, j] = NRMSE
  }
}
...
for(j in 1:(nb_series)) {
  ...
  df_real = data.frame(time = seq(1,100,length=100), Coordinate = preds_nn[[95]][[j]]$Y[1:100], 
                       Label = "Real")
  df_pred = data.frame(time = seq(1,100,length=100), Coordinate = preds_nn[[95]][[j]]$yh[1:100], 
                       Label = "Predicted")
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    obs_minmax = max(preds_n005[[i]][[j]]$Y) - min(preds_n005[[i]][[j]]$Y)
    NRMSE = sqrt(mean((preds_n005[[i]][[j]]$yh - preds_n005[[i]][[j]]$Y)^2))/obs_minmax
...
# Calculate NRMSE distributions per coordinate for smoothed prediction
NRMSEs_n005_smooth = matrix(0, nb_trials, nb_series)

for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    obs_minmax = max(preds_n005[[i]][[j]]$Y) - min(preds_n005[[i]][[j]]$Y)
    NRMSE = sqrt(mean((preds_n005[[i]][[j]]$yh_smooth - preds_n005[[i]][[j]]$Y)^2))/obs_minmax
...
for(j in 1:nb_series) {
  ...
  for(i in 1:nb_time_steps) {
    ...
    for(k in 1:nb_trials) {
      ...
      pred_aux[k, 1] = preds_n005[[k]][[j]]$Y[i] 
      pred_aux[k, 2] = preds_n005[[k]][[j]]$yh_smooth[i]
    }
...
for(j in 1:(nb_series)) {
  ...
  df_real = data.frame(time = seq(1,100,length=100), Coordinate = preds_n005[[1]][[j]]$Y[1:100],
                       Label = "Real")
  df_pred = data.frame(time = seq(1,100,length=100), Coordinate = preds_n005[[1]][[j]]$yh[1:100],
                       Label = "Predicted")
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    obs_minmax = max(preds_n01[[i]][[j]]$Y) - min(preds_n01[[i]][[j]]$Y)
    NRMSE = sqrt(mean((preds_n01[[i]][[j]]$yh - preds_n01[[i]][[j]]$Y)^2))/obs_minmax
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    obs_minmax = max(preds_n01[[i]][[j]]$Y) - min(preds_n01[[i]][[j]]$Y)
    NRMSE = sqrt(mean((preds_n01[[i]][[j]]$yh_smooth - preds_n01[[i]][[j]]$Y)^2))/obs_minmax
...
for(j in 1:nb_series) {
  ...
  for(i in 1:nb_time_steps) {
    ...
    for(k in 1:nb_trials) {
      pred_aux[k, 1] = preds_n01[[k]][[j]]$Y[i] 
      pred_aux[k, 2] = preds_n01[[k]][[j]]$yh_smooth[i]
    }
...
for(j in 1:(nb_series)) {
  ...
  df_real = data.frame(time = seq(1,100,length=100), Coordinate = preds_n01[[1]][[j]]$Y[1:100],
                       Label = "Real")
  df_pred = data.frame(time = seq(1,100,length=100), Coordinate = preds_n01[[1]][[j]]$yh[1:100],
                       Label = "Predicted")
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    obs_minmax = max(preds_n02[[i]][[j]]$Y) - min(preds_n02[[i]][[j]]$Y)
    NRMSE = sqrt(mean((preds_n02[[i]][[j]]$yh - preds_n02[[i]][[j]]$Y)^2))/obs_minmax
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    obs_minmax = max(preds_n02[[i]][[j]]$Y) - min(preds_n02[[i]][[j]]$Y)
    NRMSE = sqrt(mean((preds_n02[[i]][[j]]$yh_smooth - preds_n02[[i]][[j]]$Y)^2))/obs_minmax
...
for(j in 1:nb_series) {
  ...
  for(i in 1:nb_time_steps) {
    ...
    for(k in 1:nb_trials) {
      pred_aux[k, 1] = preds_n02[[k]][[j]]$Y[i] 
      pred_aux[k, 2] = preds_n02[[k]][[j]]$yh_smooth[i]
    }
...
for(j in 1:(nb_series)) {
  ...
  df_real = data.frame(time = seq(1,100,length=100), Coordinate = preds_n02[[1]][[j]]$yh[1:100], 
                       Label = "Real")
  df_pred = data.frame(time = seq(1,100,length=100), Coordinate = preds_n02[[1]][[j]]$yh[1:100], 
                       Label = "Predicted")
  df_pred_smooth = data.frame(time = seq(1,100,length=100), Coordinate = preds_n02[[1]][[j]]$yh_smooth[1:100], 
                       Label = "Predicted Smooth")
  
  df = rbind(df_real, df_pred, df_pred_smooth)
