...
opt_n$rho = 1e-9
#max_lag = max(opt_n$ny, opt_n$nu)
max_lag = opt_n$ny

# Choose optimal smoothing parameters
MSEs_smoothing = readRDS("Results/narx_MSEs_smoothing.RData")
...
for (i in 1:nb_trials) {
  ...
  pred_set = as.matrix(xyzu_pred[initial_points[i]:(initial_points[i]+100+max_lag), ])
  ...
  for(j in 1:nb_series) {
    for(k in 1:opt_n$ny) {
      pred_set_n005[k, j] = pred_set_n005[k, j] + 0.005*rnorm(1, 0, 1)
    }
  }
  ...
  for(j in 1:nb_series) {
    for(k in 1:opt_n$ny) {
      pred_set_n01[k, j] = pred_set_n01[k, j] + 0.01*rnorm(1, 0, 1)
    }
  }
  ...
  for(j in 1:nb_series) {
    for(k in 1:opt_n$ny) {
      pred_set_n02[k, j] = pred_set_n02[k, j] + 0.02*rnorm(1, 0, 1)
    }
  }
