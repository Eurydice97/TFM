...
gs <- list(ny = c(2, 3),
           nu = c(2, 3),
           nl = c(2, 3),
           rho = c(0.00000000001, 0.0000000001, 0.000000001, 0.00000001, 0.0000001, 0.000001))  %>% 
...
for (i in 1:nrow(gs)) {
  ...
  for (l in 1:5) {
    ...
    tryCatch({model_narx = estimate.narx(model_narx, train_set[, 1:3], as.matrix(train_set[, 4]), gs[i, 4]$rho)
    ...
    # Save to matrix
    MSEs_aux[l] = MSE_sum/nb_series}, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
...
for(j in 1:nb_series) {
  ...
  pred = data.frame(pred = pred_n005_aux[[j]]$yh, time = seq(0.001, 0.3, 0.001))
...
for(j in 1:nb_series) {
  ...
  pred = data.frame(pred = pred_n01_aux[[j]]$yh, time = seq(0.001, 0.3, 0.001))
...
for(j in 1:nb_series) {
  ...
  pred = data.frame(pred = pred_n02_aux[[j]]$yh, time = seq(0.001, 0.3, 0.001))
  
  aux = np.cv(data = as.matrix(pred), h.seq = NULL, num.h = 1000, w = NULL, num.ln = 1,
              ln.0 = 0, step.ln = 2, estimator = "LLP", kernel = "triweight")
  #print(aux$h.opt[2, ])
  #aux = np.cv(data = as.matrix(pred), h.seq = NULL, num.h = 1000, w = NULL, num.ln = 10,
  #            ln.0 = 0, step.ln = 2, estimator = "LLP", kernel = "triweight")
  #print(aux$h.opt)
