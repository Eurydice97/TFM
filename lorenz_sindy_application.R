...
for (i in 1:nb_trials) {
  ...
  for(k in 1:nb_series) {
    pred_nn_aux[[k]] = cbind(xhat_nn[, k], pred_set[, k])
    colnames(pred_nn_aux[[k]]) = c('pred', 'real')
  }
  ...
  for(k in 1:nb_series) {
    ...
    colnames(pred_n005_aux[[k]]) = c('pred', 'real')
  }
  ...
  for(k in 1:nb_series) {
    ...
    colnames(pred_n01_aux[[k]]) = c('pred', 'real')
  }
  ...
  for(k in 1:nb_series) {
    ...
    colnames(pred_n02_aux[[k]]) = c('pred', 'real')
  }
