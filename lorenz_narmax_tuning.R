...
for (i in 1:nrow(gs)) {
  ...
  for (l in 1:5) {
    ...
    tryCatch({model_narmax = estimate.narmax(model_narmax, train_set[, 1:3], as.matrix(train_set[, 4]), 
...
narmax_model = estimate.narmax(narmax_model, train_set[, 1:3], as.matrix(train_set[, 4]), 1e-12, 1e-12)
